...
export const addToQueue = (payload: {
  ...
  const { tracks, cast_info: ci } = payload;
  ...
  if (is_casting.value) {
    ...
    const cp = getCastPayload(tracks_with_index, ci);

    cast_info.value = [...cast_info.value, ...ci];

    CastSdk.Queue([], cp.map(CastSdk.Media));
  }
};
...
export const play = (payload: PlayLoad) => {
  const {
    ...
    is_virtual = false,
    skip_reload = false,
    play_options: po = { mode: PlayMode.None, more: false },
  } = payload;
  ...
  if (tracks) {
    ...
    if (is_casting.value) {
      ...
      const cp = getCastPayload(playlist.value, cast_info.value);
      const current_time = progress ? progress * now_playing.value.duration : 0;
      CastSdk.Play(cp, index.value, current_time);
    } else {
      load();
    }
  }
...
export const playNext = (payload: {
  ...
  if (is_casting.value) {
    ...
    cast_info.value = [...cast_info_head, ...ci, ...cast_info_tail];
...
export const getPlayPayload =
  (casting: boolean, tracks: Array<PlayableTrack>) => async () => {
    let ci: Maybe<PreCastPayload> = null;

    if (casting) {
      ci = await api.track.castInfo.query({
        track_ids: tracks.map((track) => track.id),
      });
    }

    return { tracks, cast_info: ci };
  };
