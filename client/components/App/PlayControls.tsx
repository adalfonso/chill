...
export const PlayControls = () => {
  ...
  const is_fullscreen =
    is_mobile &&
    playerStore.mobile_display_mode.value === MobileDisplayMode.Fullscreen;

  // Helper that gets the "now playing" section
  const getNowPlaying = () => {
    const track = playerStore.now_playing.value;
    if (!track) {
  ...
  const togglePlayer = async () =>
    playerStore.is_playing.value ? pause() : play({});

  // Stop all audio from playing
  const stop = () => {
    playerStore.setMobileDisplayMode(MobileDisplayMode.None);
    playerStore.clear();
  };

  // Minimize the fullscreen player on mobile
  const minimize = () =>
    playerStore.setMobileDisplayMode(MobileDisplayMode.Minimized);

  // Fullscreen the mobile player
  const goFullscreen = () => {
    playerStore.setMobileDisplayMode(MobileDisplayMode.Fullscreen);
  };

  const now_playing = playerStore.now_playing.value;
  const { artist_id, album_id } = playerStore.now_playing.value ?? {};
  ...
  return (
    <>
      <div
        id="play-controls"
        className={
          playerStore.mobile_display_mode.value + (is_mobile ? " mobile" : "")
        }
        style={{
          "--bg-image":
            is_mobile && playerStore.now_playing.value?.album_art_filename
              ? `url(/api/v1/media/cover/${playerStore.now_playing.value.album_art_filename}?size=500)`
              : "none",
        }}
      >
        ...
        <div className="content">
          ...
          <div className="cover-wrapper">
            <div className="cover">
              {playerStore.now_playing.value?.album_art_filename && (
                <img
                  src={`/api/v1/media/cover/${playerStore.now_playing.value?.album_art_filename}?size=500`}
                  loading="lazy"
                />
              )}
            </div>
      ...
      {playerStore.mobile_display_mode.value ===
        MobileDisplayMode.Minimized && (
        <div id="play-controls-minimized" onClick={noPropagate(goFullscreen)}>
          <div>
            {playerStore.now_playing.value?.title}
            <div className="dim">{playerStore.now_playing.value?.artist}</div>
          </div>
